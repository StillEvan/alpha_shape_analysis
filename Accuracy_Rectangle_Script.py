...
subsample_list = list(range(1450, 1501, 50))
#subsample_list = list(range(50, 101, 50))
# End user parameters

coords = np.random.uniform(low=0-1.0, high=1.0, size=(2000, 2))
new_coords = coords

plt.plot(coords[:,0], coords[:, 1], linestyle = 'none', marker = '.')
plt.show()
# Initialize storage dataframe
df = pd.DataFrame(columns = ['n_points', 'Accuracy', 'Avg_AIC', 'Std_AIC', '1 Inflection', '2 Inflection', '3 Inflection', 'Avg_Alpha', 'Std_Alpha', \
...
for i in subsample_list:
    ...
    if np.sum(temp_convexity) == 0:
        print('???')
        df2 = pd.DataFrame(np.array([i, (n_runs - np.sum(temp_convexity))/n_runs, np.mean(temp_aic), np.std(temp_aic), 0, 0, \
                       0, 0, 0, \
                       0, 0, 0, \
                       0, np.mean(temp_convex_volume[np.invert(temp_convexity)]), \
                       np.std(temp_convex_volume[np.invert(temp_convexity)]), 0, \
                       0, 0, 0, np.mean(temp_times), np.std(temp_times)]).reshape(1,21), \
                       columns = ['n_points', 'Accuracy', 'Avg_AIC', 'Std_AIC', '1 Inflection', '2 Inflection', '3 Inflection', 'Avg_Alpha', 'Std_Alpha', \
                                  'Avg_Alpha_Vol_2', 'Std_Alpha_Vol_2', 'Avg_Alpha_Vol_3', 'Std_Alpha_Vol_3', 'Avg_Convex_Vol', 'Std_Convex_Vol', 
                                  'Avg_Percent_Volume_2', 'Std_Percent_Volume_2', 'Avg_Percent_Volume_3', 'Std_Percent_Volume_3', 'Avg_Time', 'Std_Time'])
    else:
        df2 = pd.DataFrame(np.array([i, (n_runs - np.sum(temp_convexity))/n_runs, np.mean(temp_aic), np.std(temp_aic), np.sum(temp_n_inflection == 1), np.sum(temp_n_inflection == 2),\
                       ...
                       np.std(temp_percent[temp_n_inflection == 3]), np.mean(temp_times), np.std(temp_times)]).reshape(1,21), \
                       columns = ['n_points', 'Accuracy', 'Avg_AIC', 'Std_AIC', '1 Inflection', '2 Inflection', '3 Inflection', 'Avg_Alpha', 'Std_Alpha', \
                                  'Avg_Alpha_Vol_2', 'Std_Alpha_Vol_2', 'Avg_Alpha_Vol_3', 'Std_Alpha_Vol_3', 'Avg_Convex_Vol', 'Std_Convex_Vol', 
                                  'Avg_Percent_Volume_2', 'Std_Percent_Volume_2', 'Avg_Percent_Volume_3', 'Std_Percent_Volume_3', 'Avg_Time', 'Std_Time'])
